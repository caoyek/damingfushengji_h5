...
3. **输出协议契约**：
   - 输出与 `libEnvRelay.so` 中 `CBattle::ProcessBattleInfo` 100% 兼容的标准 `rounds` JSON 战报，前端播放器读取播放，彻底实现前后端职责纯正分离。
...
| **TC-06** | **新手村与剧情** | 创角完成后踏入新手村广州城 | 屏幕展现广州新手村宣纸全景底图，自动弹出二婶子对话，文本正确替换角色名。 | 待测试 |
| **TC-07** | **装备穿戴与属性** | 对话结束获得【一星晾衣杆】，点击【角色】并穿戴 | 晾衣杆从背包移至武器槽，通过接口同步，角色攻击力从 10 实时刷新为 25。 | 待测试 |
| **TC-08** | **技能装配 (3个限制)** | 打开技能面板，勾选出战技能 | 勾选超过 3 个时触发置灰拦截，出战技能列表成功更新为 `[1001]`（暗界波）。 | 待测试 |
| **TC-09** | **实机战斗与公式** | 走向城门进入过关通道，点击【市井流氓】开战 | 播放 `bgm_battle.ogg`，前端根据服务端 `rounds` 战报流畅播放冲锋、暗界波光效、扣血震屏飘字，敌人阵亡淡出，弹出胜利弹窗（播放 `win.ogg`）。 | 待测试 |
| **TC-10** | **中断恢复与持久化** | 战斗胜利回到通道后，直接按 `F5` 刷新浏览器 | 重新进入无需再次创角，直接读取 SQLite `server/daming.db` 恢复在城外通道状态，装备保持穿戴。 | 待测试 |
