...
+-----------------------------------------------------------------------------------------------+
                     ...
                     v                                                   v
+-----------------------------------------------------------------------------------------------+
|                 【核心官方基线】标准本地/远程服务 (Node.js Server + SQLite better-sqlite3)     |
|   - 存储持久化: 读写 server/daming.db (基于 26 张标准 DDL 与 4 大 baseline 官方配置源)       |
|   - 业务处理域: 创角、新手主线、装备强化、技能升级、关卡推演 (CombatSimulator.js)             |
|   - 网络通信层: 标准 HTTP POST /index.php?action=... 返回标准 JSON 数组 (100% 对齐 SO 原生协议)|
|   - 前端兼容器: 微信小游戏 (开启不校验域名) / Web 网页端秒连 http://127.0.0.1:3000           |
|                                                                                               |
|   [备用脱机回退]: LocalEngine.js (提供零服务器纯离线运行时的无缝回退兜底)                     |
+-----------------------------------------------------------------------------------------------+
```

* **SQLite 从第一天起直接实装的架构决策**：
  * **零配置、零返工**：直接采用轻量级单文件嵌入式数据库 `better-sqlite3`（`server/daming.db`），彻底规避前期使用临时内存/缓存导致后期全量推翻重构的弊端；
  * **事务原子性保证**：穿装备、技能升级扣战功、通关发奖统一使用 SQLite 事务，从根本上防止坏档与数据错乱；
  * **数据可视化与可维护性**：任何时候均可直接查询 `daming.db` 核验角色属性、怪物波次与掉落数据；
  * **原汁原味网络契约**：前端通过标准 HTTP API 发起原版 `action=login`, `action=city_dungeon_pk` 请求，端到端 100% 还原当年的网络交互时序。
